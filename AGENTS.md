...
- UIを含む作業は、コンポーネントやテストコードだけで完了扱いにせず、作業単位の途中にWebアプリを起動し、実際に表示された画面と操作を確認する工程を必ず挟む。
- 各画面の確認では、スマートフォン、タブレット、PCの代表ビューポートを用い、情報の優先順位、タップ/クリック領域、キーボード・safe area、画面回転、loading/empty/error/reconnect状態、読み取り専用表示、承認導線を実操作で検証する。
- 「実装 → ブラウザで実画面・実操作を確認 → 問題を記録 → UI/UXを改善 → 同じシナリオを再確認」の反復を、画面または主要フローごとに行う。妥当性に問題が残る場合は、次の実装単位へ進めない。
- 実画面検証は、可能な範囲でブラウザ自動操作と目視確認の両方を行い、対象URL、ビューポート、確認シナリオ、結果、改善内容をマスク済みの証跡または検証記録に残す。秘密情報、実会話本文、端末固有値は証跡へ含めない。
- fake App Serverやfixtureで確認できるUI検証と、実Codex・実モバイル端末・PWA通知など外部依存のlive検証を区別し、未実施の確認を成功扱いにしない。
