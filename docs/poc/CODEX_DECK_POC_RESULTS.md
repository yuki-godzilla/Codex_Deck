...
VS Code起点は、導入済みのVS Code 1.128.0と`openai.chatgpt@26.707.41301`で実施可能な状態を確認し、[専用手順](CODEX_DECK_VSCODE_POC_RUNBOOK.md)を追加した。Thread開始はVS Code UI操作を必要とするため、結果は未判定である。複数App Server、同一Thread競合、同一承認競合、workspace A/B並行、同一workspaceの二重開始も未検証である。
