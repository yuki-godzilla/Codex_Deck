...
現在は**P0互換性PoCとMVP実装基盤**フェーズです。Backendには、App Server stdio adapter、Bridge、workspace Scheduler、FastAPIの最小HTTP API、再接続用event storeを実装しています。Web UI、SQLite永続化、認証、WebSocket、実 App Server結合testは未実装です。
