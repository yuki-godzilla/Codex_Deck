...
現在は**P0互換性PoCとMVP実装基盤**フェーズです。Backendには、App Server stdio adapter、Bridge、workspace Scheduler、FastAPIのHTTP API、SQLite対応の再接続用event store、WebSocketイベント配信を実装しています。Web UI、SQLiteファイルの起動時構成、認証、実 App Server結合testは未実装です。
