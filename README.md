...
現在は**実装前の要件定義・公式仕様調査・PoC計画**フェーズです。アプリケーション本体、フレームワーク、依存関係、実行設定はまだ作成していません。

実装を開始する前に、Codex App Serverのstdio接続、セッション共有、workspace別並行実行、復旧、モバイルPWA、Windows運用、大規模リポジトリ性能をPoCで検証します。
