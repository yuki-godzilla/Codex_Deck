...
`backend/`には、App Serverのstdio JSON-RPC transport、Bridge契約、workspace排他Scheduler、SQLite対応Deck event store、許可root内のworkspace catalog、読み取り専用File Adapter、FastAPI HTTP/WebSocket API、およびfake App Serverによるunit testを配置した。実App Serverに対するcommand approvalの受信・同一IDへの`decline`・最小SQLite監査・障害中の自動決定/再送なしもPoCで確認した。`frontend/`には、workspace/Active work/eventを表示し依頼開始を行うReact UI縦スライスを配置した。開発時のDeck専用UI/APIポートは`43173`/`43174`で、外部originは`https://codex-deck.<tailnet-name>.ts.net`形式とする。SQLiteファイルの本番起動構成、認証、PWA、起動設定は未作成である。
...
| 2026-07-14 | Approval audit | SQLite、fake approval request | request ID、Thread/Turn/Item、kind、公式decision、時刻だけをSQLiteへ永続化することをunit testで確認。詳細本文は保存しない。 |
