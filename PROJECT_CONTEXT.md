...
| PoC-1 | 一部確認済み | stdio handshake、Thread操作、主要Item、完了、resume、file/command approvalの`decline`、実行中Turnのsteer/interrupt受理、強制終了後の再送なしを確認。再起動後のThread再読取は未確認。 |
...
1. **P0: PoC-1継続** — App Server再起動後の保存済みThread可視性と、実行中Turnを中断として扱うことを観測する。
